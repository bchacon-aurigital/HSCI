...
export const loadDevicesForAsada = async (codigoAsada: string): Promise<AsadaData> => {
  switch (codigoAsada) {
    case 'ALSFH':
      ...
      return { name: 'ASADA Los Sueños Herradura', devices: asada1Devices };
    case 'asroa2537':
    ...
    case 'villasol2026':
      ...
      return { name: 'Condominio Villa del Sol', devices: asada12Devices };
    case 'app2026':
      const { devices: asada13Devices } = await import('../app/data/devicesConfig13');
      return { name: 'ASADA Pablo Presbere', devices: asada13Devices };
    case 'ALDN2026':
      const { devices: asada14Devices } = await import('../app/data/devicesConfig14');
      return { name: 'ASADA Las Delicias de Nosara', devices: asada14Devices };
    case 'ASPB2026':
    ...
    case 'ABCN2026':
      const { devices: asada17Devices } = await import('../app/data/devicesConfig17');
      return { name: 'ASADA Barcelona', devices: asada17Devices };
    default:
